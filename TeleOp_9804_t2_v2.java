...
/* Made by the programmers of FTC Team 9804 Bomb Squad on February 6, 2016
 * v2 on 2-6-16 to
 * stevecox
 */


public class TeleOp_9804_t2_v2 extends OpMode {
    ...
    double scoreStay = 0.5;     //current position of door, initialize to UP
    double scoreFinal;
    ...
    public void loop() {
        ...
        if (gamepad1.y) {                    //when pressing the button,
            scoreFinal = scoreClosed;            //return to starting position
        }
        else{
            scoreFinal = 0.5;
        }
        ...
        if (gamepad1.a) {                           //when button is pressed,
            scoreFinal = scoreOpened;                                        //set it to the maximum point
        }

        score.setPosition(scoreFinal);           //set the servo to the position designated
